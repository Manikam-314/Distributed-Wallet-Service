...
export function SendMoneyFlow() {
  ...
  const handleConfirm = async () => {
    ...
    if (!currentPin) {
      // First time setup during transaction
      setPin(enteredPin);
    } else if (enteredPin !== currentPin) {
      setErrorStatus("Incorrect PIN. Please try again.");
      setStep("pin");
      setEnteredPin("");
      return;
    }
    ...
    try {
      ...
      if (pollIntervalRef.current) clearInterval(pollIntervalRef.current);
      const msg = e.response?.data?.error || e.response?.data?.message || e.message || "Network error. Safe to retry.";
      setErrorStatus(msg);
      setStep("failure"); 
    }
  };
          ...
          {step === "amount" && recipient && (
            <motion.div key="amount" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col h-full">
              ...
              <div className="flex-1 flex flex-col items-center justify-center relative -mt-12">
                ...
                <div className="mt-6 py-2 px-5 rounded-full bg-primary/5 border border-primary/10 text-xs font-bold text-primary flex items-center gap-2">
              ...
              <Button 
                ...
                className="w-full mt-auto mb-6 h-16 rounded-3xl text-lg font-black shadow-lg shadow-primary/20 group"
                size="lg"
              >
                Review Transfer
                <motion.div
          ...
          {step === "pin" && recipient && (
            <motion.div key="pin" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.9 }} className="space-y-6 mt-8 flex flex-col items-center">
              ...
              <h2 className="text-2xl font-black mb-1">
                {!useAuthStore.getState().pin ? "Setup Transaction PIN" : "Authorize Transfer"}
              </h2>
              <p className="text-muted-foreground text-center px-6 mb-8 text-sm font-medium">
                {!useAuthStore.getState().pin 
                  ? "Create a 6-digit PIN to secure this and future transactions." 
                  : `Sending ₹${parseFloat(amount).toLocaleString()} to ${recipient.name}`}
              </p>
              ...
              <div className="w-full flex gap-3 pb-8">
                <Button variant="outline" className="flex-1 h-14 rounded-2xl font-bold" onClick={() => setStep("confirm")}>
                  Cancel
                </Button>
                <Button 
                ...
                >
                   {!useAuthStore.getState().pin ? "Create PIN & Pay" : "Authorize"}
                </Button>
              </div>
            </motion.div>
          )}
